...
const Navbar = () => {
  ...
  return (
    <motion.nav 
      className={`fixed top-0 left-0 right-0 z-[999] px-2 sm:px-4 transition-all duration-300 ${scrolled ? 'py-1 sm:py-2' : 'py-2 sm:py-4'}`}
    ...
    >
      <div className={`max-w-xs sm:max-w-md md:max-w-3xl mx-auto backdrop-blur-md rounded-xl ${
        ...
        <div className="flex items-center justify-between h-12 sm:h-16 px-3 sm:px-6">
          ...
          <motion.div 
            ...
            whileTap={{ scale: 0.95 }}
          >
            <span className="text-[#FFFFE3] font-clash font-medium text-sm sm:text-lg md:text-2xl relative z-10 tracking-tight">
              Saturday HackNight
            </span>
            {/* Logo glow effect */}
            <motion.div 
      ...
      <span className="whitespace-pre-wrap font-clash p-1 sm:p-2 px-2 sm:px-4 text-center text-xs sm:text-sm font-medium leading-none tracking-tight text-white dark:from-white dark:to-slate-900/10 lg:text-lg">
        Register Now
      </span>
    </ShimmerButton>
        </div>
